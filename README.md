...
<b>There is one important thing to mention here: </b>When using Python SciKits (SciPy Toolkits), you will usually follow the same steps: You seperate your database into predictors (x) and targets (y), and feed them into your models. <b>Surprise, on the contrast, has its own way of working</b>. It only accepts databases built in a certain way and you're expected to keep the dataset in one piece. Also, column names must be the same as the built-in datasets. If you're used to the convenience of scikit-learn, this would cause you some discomfort. <b>Surprise also has distinct train_test_split, GridSearch, cross_validate and KFold methods</b>; these support its own working algorithm.
<h2>Methodology</h2>
